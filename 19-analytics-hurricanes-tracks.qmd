...
-   It is the sum of the maximum sustained winds ($w$) squared, when the winds are estimated every six hours
-   It is multiplied by .0001
-   $\text{ACE} = .0001 \times\sum w^2$
-   Summing ACE across each tropical cyclone throughout a season is one metric of how active the season was
...
-   'Revised' indicates post reanalysis efforts
...
>>>>>>> 54efc394d42e97f0afbfdfb5565f23433444eb0c
...
-   The International Best-Track Archive for Climate Stewardship has historical tropical cyclone data from around the world in CSV, shapefiles, and netCDF formats. These data are availe here <https://www.ncei.noaa.gov/data/international-best-track-archive-for-climate-stewardship-ibtracs/v04r00/access>
...
5.  Create a map showing the track of this hurricane
...
-   The `if()` function checks to see if the file is already in your `data` directory. If it is already there, it skips the download and unzip functions
...
-   It was the first Category 5 hurricane on record to impact the Florida Panhandle, the fourth-strongest hurricane in the contiguous United States, in terms of wind speed, and the most intense hurricane on record to strike the United States in the month of October [^1]

## {.smaller}

-   Here you remove all rows from `Track.sf` except the rows corresponding to Hurricane Michael in 2018
-   Since names are recycled every six years, you need to filter on `SEASON` and `NAME`
