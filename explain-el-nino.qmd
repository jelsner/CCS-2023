...
The values are in the file `enso_ONI.csv` in the directory `data`. 

Get the data into your R session using the `read_csv()` function from the {readr} package.

```{r}
#| echo: true
oni.df <- readr::read_csv(here::here("data", "enso_ONI.csv")) |>
  data.frame()
```
...
The SOI is a _standardized anomaly_ of the mean sea level pressure difference. The averaging (base) period is the 60 years (1933–1992).
