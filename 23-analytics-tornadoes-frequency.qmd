...
-   Time series comparing tornado alley with Dixie alley
...
Counts.df <- Torn.df |>
  dplyr::filter(mag >= 1) |>
  dplyr::mutate(Alleys = dplyr::case_when(st %in% c("KS", "OK") ~ "Tornado",
                                          st %in% c("MS", "AL") ~ "Dixie",
                                          TRUE ~ "Outside")) |>
  dplyr::group_by(Alleys, yr) |>
  dplyr::summarize(nT = dplyr::n())
...
```{r}
Counts.df |>
  dplyr::filter(!Alleys == "Outside") |>
ggplot(mapping = aes(x = yr, y = nT, color = Alleys)) +
  geom_line() +
