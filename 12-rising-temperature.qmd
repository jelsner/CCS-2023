...
-   Download the latest temperature data using the `get_temp()` function from the {hockeystick} package
...
-   Temperatures since the birth of Christ (Common Era)
-   Modeled, instrumental, filtered
...
-    Warming and emissions (Dr. Jennifer Francis)

{{< video https://www.youtube.com/embed/JKrkiZM1asQ >}} 5 minutes

-   The hockey stick graph (Simon Clark)

{{< video https://www.youtube.com/embed/CqtZdnpfgIc >}} 9 minutes
...
-     A loss of 68 and a half thousand square kilometers per year
...
ggplot(mapping = aes(x = Year, y = N100)) +
  geom_col() +
  geom_smooth(method = "glm.nb", formula = y ~ x, se = FALSE, col = "orange") +
...
-   Lapse rate of temperature (dry vs moist)
-   Parcel theory