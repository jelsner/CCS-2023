...
-   An international symbol of global warming (Dr. Ed Hawkins) {{< video https://www.youtube.com/embed/3VIHRLUnVOI >}} 3 minutes
...
-   Get the Arctic ice data using the `get_icecurves()` function from the {hockeystick} package
...
-   Make a time-series plot of September sea-ice extent
...
arcticice |>
  dplyr::filter(mo == 9) |>
ggplot(mapping = aes(x = year, y = extent)) +
  geom_line() +
  geom_smooth()
```
