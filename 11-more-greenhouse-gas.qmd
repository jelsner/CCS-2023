...
-   A greenhouse gas (GHG) absorbs and emits radiant energy within the thermal (infrared) range, causing the greenhouse effect
-   The main greenhouse gases are water vapor (H~2~O), carbon dioxide (CO~2~), methane (CH~4~), nitrous oxide (N~2~O), and ozone (O~3~)
...
-   The majority of human injected carbon dioxide comes from burning fossil fuels, coal, petroleum, and natural gas
-   Other contributing process include cement making, fertilizer production, deforestation
...
-   A copy of the file named `co2_mm_mlo.txt` is available in your `data` directory
...
-   CO~2~ values are in parts per million (by volume)
...
-   The trend does not look linear so use the default local regression smoother
...
-   Where is these gases coming from?
...
-   Compare emissions per capita between Europe and the United States over time
