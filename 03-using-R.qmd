...
* You must type in commands written in {{< fa brands r-project >}} code. In other words, you must code/program. I use the terms “coding” and “programming” interchangeably

* To begin we need some basic concepts

##

* _Console pane_: where you enter in commands
* _Running code_: the act of telling {{< fa brands r-project >}} to do something by giving it commands
* _Objects_: where values are saved. Here you _assign_ values 2 and 1, 2, 3, and 4 to objects `a` and `b`
```{r}
#| echo: true
a <- 2
b <- 1:4
```
* The symbols `<-` together are read as assign. Assign the value 2 to the object named `a`. Assign the values 1 through 4 to the object named `b`.
...
* Vectors are a series of values. These are created using the `c()` function, where `c()` is read as combine
...
* If you want only the 2nd year in the list you use brackets after the name of the object and include the integer 2 inside the brackets
```{r}
#| echo: true
Years[2]
```
...
* You will get data into R as a data frame with the function `readr::read_csv()`
```{r}
#| echo: true
...
* Logicals: `&` is read 'and' while `|` is read 'or'. For example, (2 + 1 == 3) & (2 + 1 == 4) returns FALSE since both are not TRUE (only the first is TRUE). On the other hand, (2 + 1 == 3) | (2 + 1 == 4) returns TRUE since at least one of the two clauses is TRUE.
...
* You will learn more as we go. Mastery of these concepts will build as you practice
